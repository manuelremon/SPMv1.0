...
class ReleaseMarginalCostAlgorithm(BaseAlgorithm):
    ...
    def __init__(self):
        super().__init__(AlgorithmType.RELEASE_MARGINAL_COST)
    ...
    def validate_input(self, input_data: AlgorithmInput) -> Tuple[bool, str]:
        ...
        local_stock_qty = 0.0
        if isinstance(input_data.local_stock, dict):
            local_stock_qty = sum(input_data.local_stock.values())
        elif isinstance(input_data.local_stock, (int, float)):
            local_stock_qty = float(input_data.local_stock)
    ...
    def execute(self, input_data: AlgorithmInput) -> AlgorithmOutput:
        ...
        try:
            # Si hay DB session, actualizar stock desde DB
            ...
            return AlgorithmOutput(
                algorithm_type=AlgorithmType.RELEASE_MARGINAL_COST,
    ...
    def _calculate_alternative_cost(self, input_data: AlgorithmInput) -> float:
        ...
        Transfer TDABC cost: ~2-5 por unidad (transport + picking)
    ...
    def _calculate_confidence_score(self, marginal_index: float, criticality: str) -> float:
        ...
        Base: % de ahorro marginal normalizado
        Ajuste: criticidad alta reduce confianza (más riesgo)
        """
        # Base: % de ahorro marginal
        base_confidence = min(marginal_index / 50.0, 1.0)  # Normalizado a 50%
...
# Factory function
def get_release_marginal_algorithm() -> ReleaseMarginalCostAlgorithm:
    """Obtiene instancia del algoritmo Release Marginal Cost"""
    return ReleaseMarginalCostAlgorithm()
