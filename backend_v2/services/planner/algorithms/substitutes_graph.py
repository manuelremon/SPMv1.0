...
class SubstitutesGraphAlgorithm(BaseAlgorithm):
    ...
    def execute(self, input_data: AlgorithmInput) -> AlgorithmOutput:
        ...
        try:
            ...
            if input_data.db_session:
                graph = self._fetch_equivalents_from_db(
            ...
            return AlgorithmOutput(
                ...
                estimated_cost=input_data.demand_quantity * 50.0,  # Costo estimado
            ...
            logger.error(f"Error en SubstitutesGraphAlgorithm: {e}")
            return AlgorithmOutput(
                algorithm_type=self.algorithm_type,
                item_id=input_data.item_id,
                success=False,
    ...
    def _fetch_equivalents_from_db(self, session, item_id: str) -> Dict[str, List[EquivalentNode]]:
        """Consulta equivalencias reales desde base de datos"""
        try:
            from services.planner.repositories import ItemRepository
            
            repo = ItemRepository(session)
            equivalents = repo.get_equivalents(item_id, min_technical_match=0.70)
            
            graph = {}
            # Nodo original
            graph[item_id] = [EquivalentNode(
                item_id=item_id,
                description=f"Item original {item_id}",
                technical_compatibility=1.0,
                cost_differential=0.0,
                supplier_reliability=0.95,
                lead_time_delta=0,
            ...
            for equiv in equivalents:
                graph[item_id].append(EquivalentNode(
                    item_id=equiv.equivalent_item_id,
                    description=equiv.description or f"Equivalent of {item_id}",
                    technical_compatibility=equiv.technical_match_score,
                    cost_differential=equiv.cost_difference_pct,
                    supplier_reliability=0.90,  # Default
                    lead_time_delta=equiv.lead_time_delta_days,
                    availability=0.85  # Default
                ))
            
            if not equivalents:
                self.logger.warning(f"No equivalents found for {item_id}, using simulated")
                return self._build_equivalence_graph_simulated(item_id)
            
            return graph
            
        except Exception as e:
            self.logger.warning(f"Failed to fetch equivalents from DB: {e}")
            return self._build_equivalence_graph_simulated(item_id)
    
    def _build_equivalence_graph_simulated(self, item_id: str) -> Dict[str, List[EquivalentNode]]:
        """Grafo simulado para backward compatibility"""
        return {
            item_id: [
                EquivalentNode(item_id, "Original", 1.0, 0.0, 0.95, 0, 0.6, True),
                EquivalentNode(f"{item_id}-EQV-A", "Equivalent A", 0.95, -0.05, 0.90, 2, 0.85),
                EquivalentNode(f"{item_id}-EQV-B", "Equivalent B", 0.85, 0.10, 0.92, 1, 0.92),
            ]
        }


# Factory function
def get_substitutes_graph_algorithm() -> SubstitutesGraphAlgorithm:
    """Obtiene instancia del algoritmo Substitutes Graph"""
